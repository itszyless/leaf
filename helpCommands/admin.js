  ...
  async execute(interaction) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (command.category && command.category.toLowerCase() === 'admin') {
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(adminCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = adminCommands.slice(start, start + perPage);
      const desc = slice
        .map(
          (cmd) =>
      ...
      const title = await translateText('Admin Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Admin Commands');
      embed
        .setTitle(`${title}`)
        .setDescription(desc || '—')
