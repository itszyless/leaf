...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // --- Collect all utility commands
    for (const file of files) {
      const filePath = path.join(commandsDir, file);
      try {
        const command = require(filePath);
        if (command.category && command.category.toLowerCase() === 'utils') {
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(utilsCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = utilsCommands.slice(start, start + perPage);
      const desc = slice
        .map(
          (cmd) =>
      ...
      const title = await translateText('Utility Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Utility Commands');
      embed
        .setTitle(title)
        .setDescription(desc || '—')
