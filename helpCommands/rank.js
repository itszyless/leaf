...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // --- Collect all rank commands
    for (const file of files) {
      const filePath = path.join(commandsDir, file);
      try {
        const command = require(filePath);
        if (command.category && command.category.toLowerCase() === 'rank') {
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(rankCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = rankCommands.slice(start, start + perPage);
      const desc = slice
        .map(
          (cmd) =>
      ...
      const title = await translateText('Rank Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Rank Commands');
      embed
        .setTitle(title)
        .setDescription(desc || '—')
