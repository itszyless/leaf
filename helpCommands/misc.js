...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // --- Collect all misc commands
    for (const file of files) {
      const filePath = path.join(commandsDir, file);
      try {
        const command = require(filePath);
        if (command.category && command.category.toLowerCase() === 'misc') {
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(miscCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = miscCommands.slice(start, start + perPage);
      const desc = slice
        .map((cmd) => `**/${cmd.name}** ${config['Arrow-Icon']} ${cmd.desc}`)
        .join('\n');

      const title = await translateText('Miscellaneous Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Miscellaneous Commands');
      embed
        .setTitle(title)
        .setDescription(desc || '—')
