...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(funCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = funCommands.slice(start, start + perPage);
      const desc = slice
        .map(
          (cmd) =>
      ...
      const title = await translateText('Fun Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Fun Commands');
      embed
        .setTitle(title)
        .setDescription(desc || '—')
