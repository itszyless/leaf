...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('animated')
    .setDescription('Animated commands')
  ...
  async execute(interaction) {
    ...
    const commandsDir = path.join(__dirname, '..', 'Commands'); // make sure capital C matches your folder
    const files = fs.readdirSync(commandsDir);

    const animatedCommands = [];

    // --- Collect animated commands
    for (const file of files) {
      const filePath = path.join(commandsDir, file);
      try {
        const command = require(filePath);
        if (command.category && command.category.toLowerCase() === 'animated') {
          const desc = await translateText(command.data.description, userLang);
          animatedCommands.push({
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(animatedCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = animatedCommands.slice(start, start + perPage);
      const desc = slice
        .map(
          (cmd) =>
      ...
      const title = await translateText('Animated Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Animated Commands');
      embed
        .setTitle(title)
        .setDescription(desc || '—')
