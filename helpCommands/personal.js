...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // --- Collect all personal commands
    for (const file of files) {
      const filePath = path.join(commandsDir, file);
      try {
        const command = require(filePath);
        if (command.category && command.category.toLowerCase() === 'personal') {
    ...
    // --- Pagination setup
    const perPage = 10;
    const totalPages = Math.ceil(personalCommands.length / perPage);
    let currentPage = 1;

    const renderPage = async () => {
      const start = (currentPage - 1) * perPage;
      const slice = personalCommands.slice(start, start + perPage);
      const desc = slice
        .map(
          (cmd) =>
      ...
      const title = await translateText('Personal Commands', userLang);
      const pageText = `${await translateText('Page', userLang)} ${currentPage}/${totalPages}`;

      const embed = await getUserEmbed(interaction.user.id, 'Personal Commands');
      embed
        .setTitle(title)
        .setDescription(desc || '—')
